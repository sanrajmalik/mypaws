...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
    ...
    let title = 'Buy Dogs & Puppies | Ethical Breeders India | MyPaws';
    let description = 'Find healthy, vet-checked puppies from verified ethical breeders in India. Golden Retrievers, German Shepherds, Labradors, and more.';

    if (cityName && breedName) {
        title = `Buy ${breedName} Puppies in ${cityName} | Verified Breeders`;
        description = `Find healthy ${breedName} puppies for sale in ${cityName} from ethical breeders. Vet-checked, vaccinated, and love guaranteed.`;
    } else if (breedName) {
        title = `Buy ${breedName} Puppies in India | Price, Photos & Breeders`;
        description = `Find the best ${breedName} puppies for sale in India from verified ethical breeders. Check prices, photos, and health guarantees.`;
    } else if (cityName) {
        title = `Buy Puppies in ${cityName} | Ethical Dog Breeders`;
...
export default async function BuyDogsHubPage({ searchParams }: PageProps) {
    const params = await searchParams;

    // 1. Fetch auxiliary data first to map slugs to IDs
    const [breedsRes, citiesRes] = await Promise.all([
    ...
    // 3. Fetch Listings with resolved IDs
    const listings = await searchBreederListings({
        petType: 'Dog',
        breedId: selectedBreed?.id,     // Backend uses ID
        breedSlug: params.breed,        // Pass slug just in case
        cityId: selectedCity?.id,       // Backend uses ID
        citySlug: params.city,          // Pass slug just in case
    ...
    return (
        <main className={styles['adopt-page']}>
            ...
            <div className={`${styles.container} ${styles['adopt-content']} space-y-8`}>
                ...
                <section id="listings" className="min-h-[400px]">
                    <div className="flex justify-between items-end mb-6">
                        <h2 className="text-2xl font-bold text-gray-900">
                            {selectedBreed ? `${selectedBreed.name} Puppies` : 'Puppies For Sale'}
                    ...
                    {recentListings.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {recentListings.map(listing => {
                                ...
                                return (
                                    <Link key={listing.id} href={`/buy-dogs/${listing.slug}`} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow block group">
                                        ...
                                        <div className="p-4">
                                            <h3 className="font-bold text-gray-900 group-hover:text-primary-600 truncate mb-1">{listing.title}</h3>
                        ...
                        <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
                            ...
                            <h3 className="text-lg font-medium text-gray-900">No puppies found matching your criteria</h3>
                            <p className="text-gray-500 mt-2">Try adjusting your filters to see more results.</p>
                            <Link href="/buy-dogs" className="text-primary-600 font-medium mt-4 inline-block hover:underline">
                                Clear all filters
                            </Link>
                        </div>
                    )}
                ...
                {/* Trust Signals */}
                <section className="bg-green-50 rounded-2xl p-8 md:p-10 text-center mt-12">
                    <h2 className="text-2xl font-bold text-gray-900 mb-8">Why buy from MyPaws?</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3 text-xl">🛡️</div>
                        ...
                        <div>
                            ...
                            <p className="text-gray-600 text-sm">Puppies come with vaccination records and health checks.</p>
                        </div>
                        <div>
                            <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3 text-xl">🤝</div>
                            <h3 className="font-bold text-gray-900 mb-1">Ethical Standards</h3>
                            <p className="text-gray-600 text-sm">We strictly prohibit puppy mills and unethical breeding.</p>
                        </div>
                    </div>
                </section>
