# 🚀 mypaws Production Deployment Guide
...
# Domain
NEXT_PUBLIC_BASE_URL=https://mypaws.in
...
2.  **Paste the following content** (Update `server_name` with your domain):
    ...
    server {
        listen 80;
        server_name mypaws.in www.mypaws.in;
        ...
        # Static Uploads (Optional - handled by /api/uploads in app now, but good for performance)
        location /uploads {
            alias /opt/mypaws/uploads; # Points to the volume mount on host
            expires 30d;
            access_log off;
        }
    }
...
If you visit `https://mypaws.in`, you currently hit the default Nginx site (whatomate) because `mypaws` only listens on HTTP (port 80).

To fix this and secure your site:
...
2.  **Run Certbot**:
    ```bash
    sudo certbot --nginx -d mypaws.in -d www.mypaws.in
    ```
3.  **Follow the prompts**:
    -   Enter your email.
    -   Agree to Terms.
    -   Choose "2" to Redirect HTTP to HTTPS (optional but recommended).

Certbot will automatically update your Nginx config to support HTTPS.
...
## 11. Troubleshooting: "The image has been removed" Error

If you see: `The image for the service you're trying to recreate has been removed... Continue with the new image? [yN]`

1.  **Just type `y` and Enter**.
2.  This happens when Docker cleans up old images while Compose still references them. It's harmless.
1.  **Just type `y` and Enter**.
2.  This happens when Docker cleans up old images while Compose still references them. It's harmless.
3.  To fix it permanently, run: `docker-compose -f docker-compose.prod.yml down` then `up` again.

## 12. Troubleshooting: "KeyError: 'ContainerConfig'"

If you see a crash with `KeyError: 'ContainerConfig'`, it means you are using an old version of `docker-compose` (v1) which doesn't understand the new Docker image format.

**Fix 1: Use Docker Compose V2 (Recommended)**
Try running `docker compose` (with a space) instead of `docker-compose` (with a hyphen).
```bash
docker compose -f docker-compose.prod.yml --env-file .env.prod up -d --build
```
(Most modern Ubuntu systems have V2 installed as a plugin).

**Fix 2: Manual Cleanup**
If V2 isn't available, forcing a remove will fix it:
```bash
docker rm -f mypaws_api mypaws_frontend
docker-compose -f docker-compose.prod.yml --env-file .env.prod up -d
```

## 13. Troubleshooting: "Container Name Conflict"

If you see an error like `Error response from daemon: Conflict. The container name ... is already in use by container ...`:

1.  **Copy the Container ID** from the error message (e.g., `4d140b1e7547...`).
2.  **Force remove it**:
    ```bash
    docker rm -f <CONTAINER_ID>
    ```
    (Example: `docker rm -f 4d140b1e7547d4762e8c5c7d283bb0e289ef393398a91dc7e8e41a0e7e457299`)
3.  **Run the `up` command again**.
